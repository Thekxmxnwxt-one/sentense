...
export type Room={code:string;gameId:string;game:GameInfo;stageIndex:number;phase:'lobby'|'stage_transition'|'stage_intro'|'playing'|'round_result'|'stage_complete'|'finished';players:Player[];turn:number;teamStars:number;targetStars:number;round:number;lastRoll:number;current?:Card;activePlayerId?:string;submissions:Submission[]|null;roundWinnerId?:string;roundEndsAt?:number;lobbyStartsAt?:number;message:string;winnerIds?:string[]};
